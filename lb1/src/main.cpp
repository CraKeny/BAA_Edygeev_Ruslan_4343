            ...
            if (cells[i] != fullRowMask) {
                for (int j = 0; j < size; j++) {
                    if (!(cells[i] & (1u << j))) {
                        x = i;
                        y = j;
                        std::cout << "Найдена свободная клетка " << x << ' ' << y << std::endl;
                        return true;
                    }
                }
            }
    ...
    void printBoard() const {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                if (isOccupied(i, j)) {
                    int squareSize = 0;
                    for (const auto& square : placed_squares) {
                        if (i >= square.x && i < square.x + square.size &&
                            j >= square.y && j < square.y + square.size) {
                            squareSize = square.size;
                            break;
                        }
                    }
                    std::cout << (squareSize >= 10 ? (char)('A' + squareSize - 10) : (char)('0' + squareSize)) << ' ';
                } else {
                    std::cout << ". ";
                }
            }
    ...
    while (!states.empty()) {
        ...
        if (!current.firstEmpty(x, y)) {
            if (current.placed_squares.size() < bestSolution.size()) {
                ...
                std::cout << "Отмечено, как наилучшее решение" << std::endl;
                minSquares = bestSolution.size();
            }
            continue;
        }
        
        if (current.placed_squares.size() >= bestSolution.size() || current.placed_squares.size() > (size_t)targetCount){
            std::cout << "Сценарий нереаллистичен - уже больше лучшего сценария, отброс решения" << std::endl;
            continue;
        }
        ...
        if (current.placed_squares.size() + current.emptyCellsCount() / (max_size * max_size) >= bestSolution.size()){
            std::cout << "Сценарий нереаллистичен - потенциально больше лучшего сценария, отброс решения" << std::endl;
            continue;
        }
...
void solve(int n, std::vector<Square>& bestResult) {
    ...
    for (int targetCount = 4; targetCount <= minSquares; ++targetCount) {
...
int main() {
    ...
    if (n % 2 == 0) {
        std::cout << "N - четное, частный случай (разделение на 4 равных квадрата): " << std::endl;
        int half = n / 2;
    ...
    int scale = n / divisor;
    std::cout << "N: " << n << ' ' << "Делитель: " << divisor << std::endl;

    std::vector<Square> smallResult;
    std::cout << "N - нечетное, запуск алгоритма" << std::endl;
    solve(divisor, smallResult);
    std::cout << "Алгоритма завершен, вывод результата" << std::endl;
