...
def find_pattern_with_wildcards(text, pattern, wildcard_char):
    with open(log_file, "w", encoding="utf-8") as f:
        f.write("AHO-CORASICK WILDCARD SEARCH DEBUG LOG\n")
        f.write("="*60 + "\n\n")
        f.write("INPUT DATA:\n")
        f.write(f"  Text: '{text}'\n")
    ...
    for pos, char in enumerate(text):
        ...
        patterns = ac.get_all_patterns(node)
        
        if patterns:
            with open(log_file, "a", encoding="utf-8") as f:
                f.write(f"        FOUND subpatterns: {patterns}\n")
        
        for pattern_idx, start_pos_in_pattern, length in patterns:
            pattern_start_in_text = pos - length + 1 - start_pos_in_pattern
            
            if 0 <= pattern_start_in_text < n:
                C[pattern_start_in_text] += 1
                with open(log_file, "a", encoding="utf-8") as f:
                    f.write(f"            increment C[{pattern_start_in_text}] = {C[pattern_start_in_text]}\n")
...
def main():
    ...
    positions = find_pattern_with_wildcards(text, pattern, wildcard_char)
